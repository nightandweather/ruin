...
describe("ATLAS real stellar map", () => {
  it("preserves catalogue distance under Cartesian conversion", () => {
    for (const star of CARTESIAN_STARS) expect(Math.hypot(star.xLy, star.yLy, star.zLy)).toBeCloseTo(star.distanceLy, 8);
  });

  it("maps RA 0 and declination 0 onto the positive X axis", () => {
    const point = toCartesian({ id:"test",name:"Test",raDeg:0,decDeg:0,distancePc:1,spectralType:"G",planetCount:0,kind:"landmark",source:"SIMBAD" });
    expect(point.xLy).toBeCloseTo(3.26156, 5); expect(point.yLy).toBeCloseTo(0, 8); expect(point.zLy).toBeCloseTo(0, 8);
  });
  ...
  it("keeps light time separate from hypothetical cruise time", () => {
    expect(routeMetrics(10, .1)).toEqual({ oneWaySignalYears:10, roundTripSignalYears:20, cruiseYears:100 });
  });
});
