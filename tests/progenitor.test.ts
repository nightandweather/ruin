...
describe("PROGENITOR guided self-production", () => {
  ...
  it("treats imported controllers as a real replication bottleneck", () => {
    const scarce = new ProgenitorSimulation({
      ...DEFAULT_PROGENITOR_CONFIG,
      electronicsImportKgMonth: 1_000,
    }).snapshot();
    const supplied = new ProgenitorSimulation({
      ...DEFAULT_PROGENITOR_CONFIG,
      electronicsImportKgMonth: 30_000,
    }).snapshot();
  ...
  it("increases production closure when electronics can be made locally", () => {
    const low = new ProgenitorSimulation({
      ...DEFAULT_PROGENITOR_CONFIG,
      localElectronicsPercent: 5,
    }).snapshot();
    const high = new ProgenitorSimulation({
      ...DEFAULT_PROGENITOR_CONFIG,
      localElectronicsPercent: 80,
    }).snapshot();
  ...
  it("will not replicate past the human-set ceiling", () => {
    const sim = new ProgenitorSimulation({
      ...DEFAULT_PROGENITOR_CONFIG,
      maxFactories: 2,
      electronicsImportKgMonth: 100_000,
    });
  ...
  it("shows the quality cost of an exponential replication policy", () => {
    const conservative = new ProgenitorSimulation({
      ...DEFAULT_PROGENITOR_CONFIG,
      policy: "conservative",
    }).snapshot();
    const exponential = new ProgenitorSimulation({
      ...DEFAULT_PROGENITOR_CONFIG,
      policy: "exponential",
    }).snapshot();
