...
- **ARK** — run four recycling loops for a year against the Biosphere 2 record: oxygen 20.9%→14.2% while curing concrete ate the CO₂ signal that would have flagged it. The threshold alarm fires on day 146 with the reserve nearly spent; a trend alarm — fourteen declining days is an alarm, whatever the level reads — fires on day 14. The 132-day silent window belongs to the alarm, not the failure, and closure never reaches 100 for anyone.
- **PROSPECT** — plan a mine on three surveyed deposits under the rule NI 43-101 wrote after Bre-X: an inferred resource books as zero, however many kilotonnes the map shows. The 9,000 kt "big number" is refused undrilled, drills out at 40% of its filed grade, and even then produces less than the small measured deposit — the map's biggest number was the least measured one, and low grade is an energy sentence LUMEN has to pay.
- **PATRON** — fund 24 honest studies of a 12% harm rate and watch the money choose the answer: the consortium's file drawer (37/38 positive trials published vs 3/22 negative — Turner, NEJM 2008) moves the published consensus to 8.9% with no number altered, clears a deployment worth 400 excess cases, and is undone not by purer hearts but by registered reports.
- **WAYSTATION** — run one port shift under ISS visiting-vehicle discipline: eight arrivals, a 200 m keep-out sphere, IDSS capture verification, and an emergency berth no schedule can buy. Fail one docking collar and read the answer to the port's question off the shift board: the broken tanker makes its window, and three faultless vessels miss theirs — each naming the exact shared queue that did it.
- **ASCENT** — move a foundry's 190 t/day to orbit down two roads: Edwards' 20-tonne climbers on one ribbon, and a 294-g mass driver that refuses any launch without a certified manifest, a validated corridor, and a catcher above its confidence floor. Hold the tether and watch bulk reroute while two tonnes of crew wait beside a driver with capacity to spare — the human-rated route has no N-1.
