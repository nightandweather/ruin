...
export class FleetOperationsSimulation {
  ...
  step(ticks = 1): FleetSnapshot {
    for (let index = 0; index < ticks; index += 1) {
      ...
      this.incidents.splice(
        ...
        ...this.incidents.filter((item) => item.endsAt > this.tick),
      );
      const has = (type: FleetIncident) => this.incidents.some((incident) => incident.type === type);
      this.posture =
        has("debris-strike") || this.convoyIntegrity < 75
          ? "rescue"
          : has("unknown-contact")
            ? "screen"
            : has("comms-blackout")
      ...
      if (has("debris-strike")) {
        ...
        this.convoyIntegrity -=
          this.posture === "rescue" ? this.random.range(0.02, 0.1) : this.random.range(0.3, 0.8);
  ...
  snapshot(): FleetSnapshot {
    const incidentRisk = this.incidents.reduce(
      (risk, incident) =>
        risk + (incident.type === "debris-strike" ? 22 : incident.type === "radiator-fault" ? 12 : 7),
      0,
    );
    return {
      tick: this.tick,
      posture: this.posture,
      convoyIntegrity: Number(this.convoyIntegrity.toFixed(1)),
      escortIntegrity: Number(this.escortIntegrity.toFixed(1)),
      thermalReserve: Number(this.thermalReserve.toFixed(1)),
      propellantReserve: Number(this.propellantReserve.toFixed(1)),
