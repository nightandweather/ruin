...
export function evaluateCollectorDesign(raw: CollectorDesign, fluxMultiplier = 1, deploymentFraction = 1): CollectorPerformance {
  ...
  const radiatorTemperatureK = Math.pow(wasteHeatW / (RADIATOR_EMISSIVITY * STEFAN_BOLTZMANN * design.radiatorAreaM2), 0.25);
  ...
  const totalMassKg = arrayMassKg + radiatorMassKg + shieldingMassKg + dryBusMassKg + serviceRobotsKg + design.propellantKg;
  const deliveredPowerW = grossElectricW * 0.88;
  const structuralMetalKg = totalMassKg * 0.7;
  const traceMetalKg = totalMassKg * 0.045;

  return {
    solarFluxWm2: round(solarFluxWm2, 0), incidentPowerMW: round(incidentPowerW / 1e6, 3),
    grossElectricMW: round(grossElectricW / 1e6, 3), wasteHeatMW: round(wasteHeatW / 1e6, 3),
    radiatorTemperatureK: round(radiatorTemperatureK, 1), deliveredPowerMW: round(deliveredPowerW / 1e6, 3),
    totalMassKg: round(totalMassKg, 0), structuralMetalKg: round(structuralMetalKg, 0), traceMetalKg: round(traceMetalKg, 0),
...
export function safeDeploymentFraction(design: CollectorDesign, fluxMultiplier = 1): number {
  ...
  const maxWasteW = RADIATOR_EMISSIVITY * STEFAN_BOLTZMANN * normalized.radiatorAreaM2 * MAX_RADIATOR_TEMPERATURE_K ** 4;
