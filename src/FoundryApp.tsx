...
const formatTime = (minutes: number) =>
  `SHIFT ${String(Math.floor(minutes / 60)).padStart(3, "0")}:${String(minutes % 60).padStart(2, "0")}`;
  ...
  return (
    <main className="foundry-shell">
      ...
      <section className="foundry-layout">
        <section className="foundry-main">
          <div className="foundry-metrics">
            ...
            <Metric
              label="ORDER BACKLOG"
              value={snapshot.orderBacklog}
              unit="KITS"
              alert={snapshot.orderBacklog > 0}
            />
          ...
          <div className="process-panel">
            <div className="foundry-title">
              <span>01</span> MATERIAL TRANSFORMATION LINE
            </div>
                ...
                <div className="process-wrap" key={stage.id}>
                  <article className={`process-card ${stage.status}`}>
                    ...
                    <div className="utilization">
                      <i style={{ width: `${stage.utilizationPercent}%` }} />
                    </div>
                    <footer>
                      <span>LOAD {stage.utilizationPercent}%</span>
                      <span>HLTH {stage.healthPercent}%</span>
                    </footer>
                  </article>
          ...
          <div className="inventory-panel">
            <div className="foundry-title">
              <span>02</span> MASS BALANCE
            </div>
            <div className="inventory-grid">
              <Inventory
                label="RAW REGOLITH"
                value={snapshot.inventory.regolithKg}
                unit="kg"
                color="#a48d72"
              />
              <Inventory
                label="GRADED FEED"
                value={snapshot.inventory.gradedFeedKg}
                unit="kg"
                color="#d2b276"
              />
              <Inventory
                label="STRUCTURAL METAL"
                value={snapshot.inventory.structuralMetalKg}
                unit="kg"
                color="#b7c8cb"
              />
              <Inventory
                label="TRACE METALS"
                value={snapshot.inventory.rareMetalKg}
                unit="kg"
                color="#e6cc70"
              />
              <Inventory
                label="OXYGEN CO-PRODUCT"
                value={snapshot.inventory.oxygenKg}
                unit="kg"
                color="#67b9ff"
              />
              <Inventory
                label="MACHINED SETS"
                value={snapshot.inventory.machinedSets}
                unit="sets"
                color="#64f3c2"
              />
            </div>
            <div className="science-note">
              MRE yields and all throughput values are configurable scenario parameters—not lunar resource
              predictions.
            </div>
          </div>
        </section>
        ...
        <aside className="foundry-side">
          <div className="foundry-title">
            <span>03</span> PRODUCTION CONTROL
          </div>
          <button className="order-button" onClick={() => setSnapshot(simulation.requestKits(20))}>
            <span>+20</span> REQUEST REPAIR KITS
          </button>
          ...
          <div className="foundry-title event-title">
            <span>04</span> FACTORY LEDGER
          </div>
      ...
      <footer className="foundry-controls">
        <div>
          <button className="run" onClick={() => setRunning((value) => !value)}>
            {running ? "Ⅱ PAUSE" : "▶ RUN"}
          </button>
          {[1, 3, 12].map((value) => (
...
function Metric({
  ...
  return (
    <div className={`foundry-metric ${alert ? "alert" : ""} ${wide ? "wide" : ""}`}>
      <span>{label}</span>
      <strong>
        {value}
        <small>{unit}</small>
...
function Inventory({
  ...
  return (
    <div className="inventory-item" style={{ "--material": color } as React.CSSProperties}>
      <i />
      <span>{label}</span>
      <strong>
        {number(value, 1)} <small>{unit}</small>
      </strong>
    </div>
  );
}
