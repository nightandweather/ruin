    ...
    <main className="collector-shell">
      ...
      <section className="collector-layout">
        <aside className="design-panel panel-box">
          ...
          <Range
            label="ORBIT RADIUS"
            value={design.orbitAu}
            min={0.2}
            max={1.5}
            step={0.05}
            unit="AU"
            onChange={(v) => update("orbitAu", v)}
          />
          <Range
            label="COLLECTOR AREA"
            value={design.collectorAreaM2}
            min={400}
            max={4000}
            step={100}
            unit="m²"
            onChange={(v) => update("collectorAreaM2", v)}
          />
          <Range
            label="RADIATOR AREA"
            value={design.radiatorAreaM2}
            min={200}
            max={2000}
            step={50}
            unit="m²"
            onChange={(v) => update("radiatorAreaM2", v)}
          />
          <Range
            label="CONVERSION"
            value={design.conversionEfficiency * 100}
            min={15}
            max={55}
            step={1}
            unit="%"
            onChange={(v) => update("conversionEfficiency", v / 100)}
          />
          <Range
            label="SHIELD DEPTH"
            value={design.shieldThicknessMm}
            min={1}
            max={20}
            step={1}
            unit="mm"
            onChange={(v) => update("shieldThicknessMm", v)}
          />
          <Range
            label="PROPELLANT"
            value={design.propellantKg}
            min={200}
            max={2500}
            step={50}
            unit="kg"
            onChange={(v) => update("propellantKg", v)}
          />
          <div className="material-bill">
            <span>FOUNDRY BILL</span>
            <div>
              <b>{number(snapshot.performance.structuralMetalKg)}</b>
              <small>kg structural</small>
            </div>
            <div>
              <b>{number(snapshot.performance.traceMetalKg)}</b>
              <small>kg trace metals</small>
            </div>
            <div>
              <b>{snapshot.performance.foundryShifts}</b>
              <small>factory shifts</small>
            </div>
          </div>
        </aside>
        ...
        <section className="machine-panel panel-box">
          <div className="machine-heading">
            <Title number="02" text="C-01 SEED COLLECTOR" />
            <span className={`verdict ${verdict.toLowerCase().replaceAll(" ", "-")}`}>{verdict}</span>
          </div>
          ...
          <div className="schematic-legend">
            ...
            <span>NOT TO SCALE · GEOMETRY FOLLOWS DESIGN RATIOS</span>
          </div>
        </section>
        ...
        <aside className="telemetry-panel panel-box">
          ...
          <div className="telemetry-grid">
            ...
            <Metric
              label="RADIATOR"
              value={snapshot.performance.radiatorTemperatureK}
              unit="K"
              alert={snapshot.performance.thermalMarginK < 25}
            />
          ...
          <div className="consumables">
            <div>
              <span>Xe/Kr RESERVE</span>
              <b>{number(snapshot.propellantRemainingKg, 1)} kg</b>
            </div>
            <div>
              <span>REPAIR KITS</span>
              <b>{snapshot.maintenanceKits}</b>
            </div>
          </div>
          <p className="model-note">
            Solar flux follows inverse square distance. Efficiency, areal density, transmission loss,
            shielding mass, and factory throughput are RUIN scenario parameters.
          </p>
        </aside>

        <section className="operations-panel panel-box">
          <div className="incident-controls">
            <Title number="04" text="INCIDENT INJECTION" />
            <div>
              {(Object.keys(incidents) as CollectorIncident[]).map((type) => {
                const item = incidents[type];
                const active = snapshot.activeIncidents.some((incident) => incident.type === type);
                return (
                  <button
                    key={type}
                    disabled={active}
                    className={active ? "active" : ""}
                    onClick={() => setSnapshot(simulation.inject(type))}
                  >
                    <b>{item.code}</b>
                    <span>
                      {item.title}
          ...
          <div className="collector-events">
            <Title number="05" text="AUTONOMY LEDGER" />
            <div>
      ...
      <footer className="collector-controls">
        <div>
          ...
          <button disabled={running} onClick={() => setSnapshot(simulation.step())}>
            STEP
          </button>
          <button onClick={reset}>RESET DESIGN</button>
        </div>
        <span>
          THERMAL MARGIN {snapshot.performance.thermalMarginK > 0 ? "+" : ""}
          {snapshot.performance.thermalMarginK} K · DEPLOYED {snapshot.deploymentPercent}%
        </span>
      </footer>
    </main>
  );
...
function CollectorSchematic({
  design,
  deployment,
  mode,
}: {
  design: CollectorDesign;
  deployment: number;
  mode: string;
}) {
  ...
  return (
    <svg
      className={`collector-schematic ${mode}`}
      viewBox="0 0 720 470"
      role="img"
      aria-label="Parametric diagram of the C-01 seed collector"
    >
      <defs>
        <linearGradient id="arrayFill" x1="0" y1="0" x2="1" y2="1">
          <stop stopColor="#85631f" />
          <stop offset=".5" stopColor="#e7bd52" />
          <stop offset="1" stopColor="#5f4519" />
        </linearGradient>
        <filter id="glow">
      ...
      <circle cx="360" cy="235" r="190" className="orbit-ring" />
      <path d="M74 67L145 67" className="sun-rays" />
      <circle cx="58" cy="67" r="13" className="diagram-sun" filter="url(#glow)" />
      <text x="35" y="99">
        SOL · {design.orbitAu.toFixed(2)} AU
      </text>
      <g className="machine-geometry" transform={`translate(360 235) scale(${scale},1)`}>
        {[
          ...
          <g key={index} transform={`translate(${x * 46} ${y * 25})`}>
            <line x1="0" y1="0" x2={x * 32} y2={y * 18} className="boom" />
            <rect
              x={x < 0 ? -wingLength - 30 : 30}
              y={y < 0 ? -wingWidth - 18 : 18}
              width={wingLength}
              height={wingWidth}
              className="collector-wing"
            />
            <path
              d={`M${x < 0 ? -wingLength - 30 : 30 + wingLength / 2},${y < 0 ? -wingWidth - 18 : 18}v${wingWidth}`}
        ...
        <rect
          x={-radiatorLength / 2}
          y="-144"
          width={radiatorLength}
          height="58"
          className="radiator-panel"
        />
        <rect x={-radiatorLength / 2} y="86" width={radiatorLength} height="58" className="radiator-panel" />
      </g>
      <g className="central-bus">
        <rect x="331" y="171" width="58" height="128" rx="4" />
        <rect x="343" y="187" width="34" height="34" className="core" />
        <circle cx="360" cy="250" r="12" />
        <path d="M339 278h42" />
        <text x="325" y="327">
          SERVICE SPINE
        </text>
      ...
      <g className="thrusters">
        <path d="M345 299l-11 24h22z" />
        <path d="M375 299l-11 24h22z" />
      </g>
      <g className="service-bots">
        <circle cx="307" cy="235" r="7" />
        <circle cx="413" cy="235" r="7" />
        <path d="M300 235h-18M420 235h18" />
      </g>
      <path d="M389 206C470 179 535 184 641 159" className="power-beam" />
      <text x="538" y="147">
        POWER CORRIDOR
      </text>
      <g className="dimension">
        <path d={`M${360 - wingLength - 78} 415H${360 + wingLength + 78}`} />
        <path d={`M${360 - wingLength - 78} 409v12M${360 + wingLength + 78} 409v12`} />
        <text x="314" y="441">
          {number(design.collectorAreaM2)} m² ACTIVE AREA
        </text>
      </g>
    </svg>
  );
...
function Title({ number, text }: { number: string; text: string }) {
  return (
...
function Range({
  ...
  const set = (next: number) => onChange(Math.max(min, Math.min(max, Number(next.toFixed(4)))));
  return (
    <label className="design-range">
      <span>
        {label}
        <b>
          {number(value, step < 1 ? 2 : 0)} <small>{unit}</small>
        </b>
      </span>
...
function Metric({
  ...
  return (
    <div className={`collector-metric ${accent ? "accent" : ""} ${alert ? "alert" : ""}`}>
      <span>{label}</span>
      <b>
        {value}
        <small>{unit}</small>
