...
export type GravitasIncident = "mass-imbalance" | "bearing-loss" | "pressure-sector" | "spin-drive" | "vestibular-event";
...
export const ARCHITECTURE_PRESETS: Record<GravityArchitecture, Partial<GravitasConfig>> = {
  ring: { radiusM: 120, targetG: 0.38, rotatingMassTonnes: 2_800, crew: 96, spinupHours: 24, counterRotationPercent: 95 },
  tether: { radiusM: 350, targetG: 0.38, rotatingMassTonnes: 620, crew: 24, spinupHours: 18, counterRotationPercent: 0 },
  "short-arm": { radiusM: 8, targetG: 1, rotatingMassTonnes: 42, crew: 4, spinupHours: 0.5, counterRotationPercent: 100 },
  "field-core": { radiusM: 12, targetG: 1, rotatingMassTonnes: 80, crew: 12, spinupHours: 1, counterRotationPercent: 100 },
};
...
export class GravitasSimulation {
  ...
  updateConfig(config: GravitasConfig) { this.config = { ...config }; return this.snapshot(); }
  ...
  inject(type: GravitasIncident) {
    ...
    this.record(type === "mass-imbalance" || type === "pressure-sector" ? "critical" : "warning", messages[type]);
  ...
  step(ticks = 1) { this.tick += ticks; return this.snapshot(); }

  snapshot(): GravitasSnapshot {
    const c = this.config;
    if (c.architecture === "field-core") return {
      tick: this.tick, mode: "unsupported", feasibility: "UNSUPPORTED", rpm: null, angularVelocityRadS: null, rimSpeedMS: null,
      footG: null, headG: null, gravityGradientPercent: null, coriolisG: null, spinEnergyGJ: null, spinupPowerMW: null,
      effectiveStressMPa: null, residualAngularMomentumMNs: null, comfort: "UNKNOWN", readiness: "NO-GO",
      activeIncidents: [...this.incidents], events: this.events.map(e => ({ ...e })),
    };
    ...
    let comfort: GravitasSnapshot["comfort"] = rpm <= 2 && gradient <= 5 && coriolisG <= 0.03 ? "GOOD" : rpm <= 4 && gradient <= 15 && coriolisG <= 0.08 ? "ADAPTATION" : "HIGH-RISK";
    if (this.incidents.has("vestibular-event")) comfort = "HIGH-RISK";
    let readiness: GravitasSnapshot["readiness"] = comfort === "GOOD" && stressMPa < 120 && residualMomentum < 4_000 ? "READY" : "CONDITIONAL";
    if (stressMPa > 240 || c.targetG <= 0 || c.targetG > 1.5 || this.incidents.has("mass-imbalance")) readiness = "NO-GO";
    ...
    return {
      tick: this.tick, mode, feasibility: c.architecture === "short-arm" ? "PHYSICAL" : "EXTRAPOLATED",
      rpm: +rpm.toFixed(2), angularVelocityRadS: +omega.toFixed(3), rimSpeedMS: +rimSpeed.toFixed(1), footG: +c.targetG.toFixed(2), headG: +headG.toFixed(2),
      gravityGradientPercent: +gradient.toFixed(1), coriolisG: +coriolisG.toFixed(3), spinEnergyGJ: +(energyJ / 1e9).toFixed(1),
      spinupPowerMW: +(spinPowerW / 1e6).toFixed(2), effectiveStressMPa: +stressMPa.toFixed(1), residualAngularMomentumMNs: +residualMomentum.toFixed(0),
      comfort, readiness, activeIncidents: [...this.incidents], events: this.events.map(e => ({ ...e })),
    };
  }
