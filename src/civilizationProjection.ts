...
export function projectCivilization(snapshot: SimulationSnapshot) {
  const deliveredRatio = snapshot.metrics.demandGW === 0 ? 1 : snapshot.metrics.deliveredGW / snapshot.metrics.demandGW;
  ...
  const stress = Math.min(1, Math.max(0,
    (1 - deliveredRatio) * 1.1 + isolatedRatio + offlineRatio * 2.4 + thermalRatio * 0.45,
  ));
  const horizons = [
    ...
    const tone = population < 5.5 || trust < 32 ? "irreversible" : population < 7.4 || trust < 60 ? "uncertain" : "nominal";
